...
module.exports = title => {
  ...
  const components = title
    ...
    .replace(/\[[practicl]{8,}.[exrcis]{7,}]/i, 'practical-exercise')
    .replace(/ \/ /g, '/')
    .replace(/([^ \n\r[(])(\[|\()/g, '$1 $2')
    .replace(/(\]|\))([^ \n\r\])])/g, '$1 $2')
    .split(/ /g)
  ...
  for (let compo of components) {
    if (compo.match(/\d+[-/]\d+[-/]\d+/)) {
      ...
      flow.push({
        value: compo.replace(/[[()\]]/g, '').replace(/\//g, '-'),
        type: 'difficulty'
      })
    } else {
      flow.push({
  ...
  if (score <= 6) {
    ...
    if (props.title) {
      props.title = props.title.replace(/^'? ?\((.*)\) ?'?$/m, '$1')
    }
    return props
  } else {
    return false
